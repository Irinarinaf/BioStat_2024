...
```{r bars task 2, conclusion that both Gryffindor and Slytherin has highest percentage of pure-blood students, but Gfyffindor has more than twice higher percentage of muggle-borns compared with Slytherin, and the highest among all houses}
...
```{r bars task 3}
hogwarts_puremuggle <- hogwarts |> filter(bloodStatus %in% c('pure-blood', 'muggle-born'))
#hogwarts_puremuggle |> glimpse()

ggplot(data = hogwarts_puremuggle) + 
  geom_bar(aes(x = house, fill = bloodStatus), position = "fill") + 
  theme_light() +
  theme_custom +
...
```{r boxplots tasks 1-3, fig.width=14, fig.height=14}
...
```{r other task 1, fig.width=14, fig.height=25}
